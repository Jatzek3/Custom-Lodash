...
test('Adds a value to Array', () =>{
    expect(_.addToArray([1,2,3],4)).toStrictEqual([1,2,3,4]);
});

test('split an array in to parts of given length', () => {
    expect(_.chunk([1,2,3,4],3)).toStrictEqual([[1,2,3],[4]]);
});

test('Creates an array with all falsey values removed. The values `false`, `null`', () =>{
    expect(_.compact([0,1,2,3,4,null])).toStrictEqual([1,2,3,4]);
});

test('Creates a slice of `array` with `n` elements dropped from the beginning', () => {
    expect(_.drop([1,2,3,4,5,6],3)).toStrictEqual([4,5,6]);
});
...
// <------------------------------------------------------------>
// Objects
// test('sets a key Value Pair for an object', 
// () => {
//     let obj ={};
//     expect(_.setProperty(obj, 'a', 5))
//     .toMatchObject({'a' : 5})
// }),
// 
